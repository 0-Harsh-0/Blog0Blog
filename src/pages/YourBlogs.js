  ...
  useEffect(() => {
    ...
    const getBlogs = onSnapshot(collection(db, "blogs"), (snapshot) => {
      ...
      list.forEach((item, index) => {
        if(item.userId === obj.userData.uid)
        {
        ...
        <AppBar position='static' sx={{ width: "auto", margin: "1rem", background: purple[700], borderRadius: ".3rem", borderTop: "1px solid white", borderLeft: "1px solid white", padding: ".3rem" }}>
          <Grid container sx={{ justifyContent: "center", alignItems: 'center' }}>
            {categoryOption.map((item, index) => {
              return <Grid item key={obj.userData.uid+index}>
                <Button className={location.pathname === `/yourblogs/${item.toLowerCase()}` ? "activeBlog" : "notActiveBlog"} component={Link} to={`/yourblogs/${item.toLowerCase()}`} sx={{ color: "white" }} onClick={() => { obj.changeOtherBlogUrl(item.toLowerCase()) }}>{item}</Button></Grid>
